        ...
        if(passed){
            for(int i=0; i < tutors.size(); i++){
                ...
                if((usernameInput.equals(tempTutor.getUsername())) && (passwordInput.equals(tempTutor.getPassword()))) {
                    PrintWriter pw = new PrintWriter("src/resources/currUserFile.txt");
                    pw.close();

                    for(int k = 0; k < tempTutor.getTutorCourses().size(); k++){
                        tempC = tempTutor.getTutorCourses().get(k);
                        if (once) {
                    ...
                    //Write Logged In User Data to Random Access File
                    try (BufferedWriter locFile = new BufferedWriter(new FileWriter("src/resources/currUserFile.txt", true))) {             //Write data to file
                        locFile.write(tempTutor.getTutorName() + "~" + courseStr + "~" + tempTutor.getTutorGrade() + "~" + tempTutor.getSerialCode() + "~" + tempTutor.getEmail() + "~" + tempTutor.getPhoneNumber() + "~" + tempTutor.getUsername() + "~" + tempTutor.getPassword() + "\n" );
                    }
